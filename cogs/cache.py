...
from .config import CacheConfig as cfg, FETCH_INTERVAL
...
class CDNCache():
    ...
    def __init__(self):
        self.cache_path = os.path.join(self.SELF_PATH, cfg.CACHE_FOLDER_NAME)
        self.data_path = os.path.join(self.cache_path, cfg.CACHE_FILE_NAME)
        ...
        if load_watchlist:
            ...
            self.watchlist = cfg.defaults.WATCHLIST
            self.save_watchlist()
    ...
    def init_json(self):
        ...
        with open(self.data_path, "w") as file:
            template = {
                cfg.indices.BUILDINFO: {},
                cfg.indices.WATCHLIST: {857764832542851092: cfg.defaults.WATCHLIST},
                cfg.indices.LAST_UPDATED_BY: self.PLATFORM,
                cfg.indices.LAST_UPDATED_AT: time.time()
            }
    ...
    def add_to_watchlist(self, branch:str, guild_id:int):
        ...
        if branch not in cfg.PRODUCTS.keys():
            return cfg.strings.BRANCH_NOT_VALID
        else:
            if guild_id in self.watchlist.keys():
                if branch in self.watchlist[guild_id]:
                    return cfg.strings.BRANCH_ALREADY_IN_WATCHLIST
                else:
    ...
    def remove_from_watchlist(self, branch:str, guild_id:int):
        ...
        if guild_id in self.watchlist.keys():
            if branch not in self.watchlist[guild_id]:
                raise ValueError(cfg.strings.ARG_BRANCH_NOT_ON_WATCHLIST)
            else:
    ...
    def load_watchlist(self):
        ...
        with open(self.data_path, "r") as file:
            file = json.load(file)
            if not cfg.indices.LAST_UPDATED_BY in file:
                file[cfg.indices.LAST_UPDATED_BY] = self.PLATFORM

            if not cfg.indices.LAST_UPDATED_AT in file:
                file[cfg.indices.LAST_UPDATED_AT] = time.time()

            if not cfg.indices.CHANNELS in file:
                file[cfg.indices.CHANNELS] = {}

            return file[cfg.indices.WATCHLIST], file[cfg.indices.CHANNELS]
    ...
    def save_watchlist(self):
        ...
        with open(self.data_path, "r+") as file:
            ...
            file_json[cfg.indices.WATCHLIST] = self.watchlist
            file_json[cfg.indices.CHANNELS] = self.channels
            file_json[cfg.indices.LAST_UPDATED_BY] = self.PLATFORM
            file_json[cfg.indices.LAST_UPDATED_AT] = time.time()
    ...
    def compare_builds(self, branch:str, newBuild:dict) -> bool:
        ...
        with open(self.data_path, "r") as file:
            ...
            if file_json[cfg.indices.LAST_UPDATED_BY] != self.PLATFORM and (time.time() - file_json[cfg.indices.LAST_UPDATED_AT]) < (FETCH_INTERVAL* 60):
                logger.info("Skipping build comparison, data is outdated")
                return False

            for area in cfg.AREAS_TO_CHECK_FOR_UPDATES:
                if branch in file_json[cfg.indices.BUILDINFO]:
                    if file_json[cfg.indices.BUILDINFO][branch][area] != newBuild[area]:
                        return True
                    else:
                        return False
                else:
                    file_json[cfg.indices.BUILDINFO][branch][area] = newBuild[area]
                    return True
    ...
    def save_build_data(self, branch:str, data:dict):
        ...
        with open(self.data_path, "r+") as file:
            file_json = json.load(file)
            file_json[cfg.indices.BUILDINFO][branch] = data
    ...
    def load_build_data(self, branch:str):
        ...
        with open(self.data_path, "r") as file:
            file_json = json.load(file)
            if branch in file_json[cfg.indices.BUILDINFO]:
                return file_json[cfg.indices.BUILDINFO][branch]
            else:
                file_json[cfg.indices.BUILDINFO][branch] = {
                    cfg.indices.REGION: cfg.defaults.REGION,
                    cfg.indices.BUILD: cfg.defaults.BUILD,
                    cfg.indices.BUILDTEXT: cfg.defaults.BUILDTEXT
                }
                return False

    async def fetch_cdn(self):
        """This is a disaster."""
        logger.debug(cfg.strings.LOG_FETCH_DATA)
        async with httpx.AsyncClient() as client:
            new_data = []
            for branch in cfg.PRODUCTS:
                try:
                    logger.debug(f"Grabbing data for branch: {branch}")
                    url = cfg.CDN_URL + branch + "/versions"

                    res = await client.get(url, timeout=20)
                    logger.debug(cfg.strings.LOG_PARSE_DATA)
